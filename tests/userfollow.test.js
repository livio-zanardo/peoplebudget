    ...
    it("should give error missing param 'followedUserId'", async (done) => {
        ...
        request(app)
            ...
            .then(({ body, statusCode }) => {
                ...
                expect(body.error).toBe("Missing paramater 'followedUserId' .");
    ...
    it('should delete one userfollow', async (done) => {
        request(app)
            ...
            .then(({ body, statusCode }) => {
                expect(statusCode).toEqual(200);
                expect(body).toHaveProperty('response');
                expect(body.response).toBe(`userfollow ${testUserFollowID} deleted`);
                done();
            });
    });
    ...
    it('should give error missing param id', async (done) => {
        request(app)
            ...
            .then(({ body, statusCode }) => {
                expect(statusCode).toEqual(400);
                expect(body).toHaveProperty('error');
                expect(body.error).toBe(`id ${testJsonDataResponse.id} doesn't exist`);
                expect(body).toHaveProperty('type');
                expect(body.type).toBe('ClientError');
                done();
            });
